...
#TODO, choose range of lambda based on fit first, and then optimize mse
```
...
ridge_2 <- glmnet(X, as.vector(Avg_ExtentTS), alpha=0, lambda=2.037965)
...
season <- factor(cycle(Avg_ExtentTS))
X <- model.matrix(as.vector(Avg_ExtentTS)~tim+season)
lasso <- glmnet(X, as.vector(Avg_ExtentTS), alpha=1, lambda=0.005670758)
...
season <- factor(cycle(Avg_ExtentTS))
X <- model.matrix(as.vector(Avg_ExtentTS)~poly(tim,2)+season)
lasso <- glmnet(X, as.vector(Avg_ExtentTS), alpha=1, lambda=0.373098)
lasso_fitted <- predict(lasso, X, type="response")
lasso_residuals <- Avg_ExtentTS - lasso_fitted
plot(lasso_residuals, type="l")
plot(Avg_ExtentTS)
...
c(model_1_train$AIC, model_1_train$AICc, model_1_train$BIC)
```
...
c(model_2_train$AIC, model_2_train$AICc, model_2_train$BIC)
```
...
c(model_3_train$AIC, model_3_train$AICc, model_3_train$BIC)
```
```{r}
...
c(model_4_train$AIC, model_4_train$AICc, model_4_train$BIC)
```
...
c(model_5_train$AIC, model_5_train$AICc, model_5_train$BIC)
```

```{r}
#SARIMA(4,1,0)x(1,1,1)_12
model_6_train <- sarima(Avg_ExtentTS_Train, p=4, d=1, q=0, P=1, D=1, Q=1, S=12 , details = TRUE)
model_6_train_residuals = resid(model_4_train$fit)
hist(model_6_train_residuals)
shapiro.test(model_6_train_residuals)
c(model_6_train$AIC, model_6_train$AICc, model_6_train$BIC)
```

```{r}
#SARIMA(1,1,1)x(0,1,1)_12
model_7_train <- sarima(Avg_ExtentTS_Train, p=1, d=1, q=1, P=0, D=1, Q=1, S=12 , details = TRUE)
model_7_train_residuals = resid(model_7_train$fit)
hist(model_7_train_residuals)
shapiro.test(model_7_train_residuals)
c(model_7_train$AIC, model_7_train$AICc, model_7_train$BIC)
```

```{r}
#SARIMA(1,1,1)x(3,1,0)_12
model_8_train <- sarima(Avg_ExtentTS_Train, p=1, d=1, q=1, P=3, D=1, Q=0, S=12 , details = TRUE)
model_8_train_residuals = resid(model_8_train$fit)
hist(model_8_train_residuals)
shapiro.test(model_8_train_residuals)
c(model_8_train$AIC, model_8_train$AICc, model_8_train$BIC)
```

```{r}
#SARIMA(1,1,1)x(1,1,1)_12
model_9_train <- sarima(Avg_ExtentTS_Train, p=1, d=1, q=1, P=1, D=1, Q=1, S=12 , details = TRUE)
model_9_train_residuals = resid(model_4_train$fit)
hist(model_9_train_residuals)
shapiro.test(model_9_train_residuals)
c(model_9_train$AIC, model_9_train$AICc, model_9_train$BIC)
```
...
goodness_of_fit %>% 
  ...
  set_bold(9, everywhere) %>%
  set_background_color(9, everywhere, "grey95")
```

## Model Selection

```{r}
model_1_train_forecast<- sarima.for(Avg_ExtentTS_Train, n.ahead=22, p=0,d=1,q=2,P=0,D=1,Q=1,S=12)
model_4_train_forecast<- sarima.for(Avg_ExtentTS_Train, n.ahead=22, p=4,d=1,q=0,P=0,D=1,Q=1,S=12)
model_8_train_forecast<- sarima.for(Avg_ExtentTS_Train, n.ahead=22, p=1,d=1,q=1,P=3,D=1,Q=0,S=12)

mean((model_1_train_forecast$pred-Avg_ExtentTS_Test)^2)
mean((model_3_train_forecast$pred-Avg_ExtentTS_Test)^2)
